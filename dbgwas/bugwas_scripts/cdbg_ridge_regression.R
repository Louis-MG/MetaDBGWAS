...
## file BUGWAS_ridge_regression.R. Appending "cdbg" to the modified function.
...
## Changelog:
##
## - Use pcaX instead of svdX argument.
## - Remove X = x duplication.
## - Remove zero padding of null space (requires duplication of rotation matrix, takes up space)
## - Replace svdR %*% t(svdR) by tcrossprod(svdR), t(svdR) %*% svdR by crossprod(svdR)
################################################################################################

cdbg_ridge_regression = function(y,x,w=matrix(1,length(y),1),K_type="uncentred",pca,lambda_init=200,prefer.REML=TRUE,skip.var=FALSE,maximize=TRUE) {
    ...
    svdR <- pca$x # (Equivalent to above formula)
    ...
    if(skip.var) {
        ...
        post.var = lambda/tau * (1 - colSums(t(pca$rotation^2)*diag(Cstar)))
    }
    
    ret = list("lambda_MLE"=lambda_MLE,"lambda_REML"=lambda_REML,"prefered.lambda"=lambda, "a_MLE"=a_MLE,"tau_MLE"=tau_MLE[1,1],"tau_REML"=tau_REML[1,1],"Ebeta"=post.mean,"Vbeta"=post.var,"ML"=ML,"REML"=REML,"func"=function(x){x})
    return(ret)
}
